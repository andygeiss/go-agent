...
func Test_ToolCall_NewToolCall_With_ValidParams_Should_ReturnPendingToolCall(t *testing.T) {
	// Arrange
	id := immutable.ToolCallID("tc-1")
	name := "search"
	args := `{"query": "test"}`

	// Act
	tc := entities.NewToolCall(id, name, args)
	...
	assert.That(t, "tool call status must be pending", tc.Status, immutable.ToolCallStatusPending)
}

func Test_ToolCall_Complete_With_Result_Should_BeCompleted(t *testing.T) {
	// Arrange
	tc := entities.NewToolCall("tc-1", "search", `{}`)
	tc.Execute()
	...
	// Assert
	assert.That(t, "tool call status must be completed", tc.Status, immutable.ToolCallStatusCompleted)
...
func Test_ToolCall_Execute_With_PendingToolCall_Should_BeExecuting(t *testing.T) {
	...
	// Assert
	assert.That(t, "tool call status must be executing", tc.Status, immutable.ToolCallStatusExecuting)
}
...
func Test_ToolCall_Fail_With_Error_Should_BeFailed(t *testing.T) {
	...
	// Assert
	assert.That(t, "tool call status must be failed", tc.Status, immutable.ToolCallStatusFailed)
...
func Test_ToolCall_ToMessage_With_CompletedToolCall_Should_ReturnToolMessage(t *testing.T) {
	...
	// Assert
	assert.That(t, "message role must be tool", msg.Role, immutable.RoleTool)
