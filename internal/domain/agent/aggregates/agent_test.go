...
func Test_Agent_NewAgent_With_ValidParams_Should_Return_Agent(t *testing.T) {
	// Arrange
	id := immutable.AgentID("test-agent")
	systemPrompt := "You are a helpful assistant."

	// Act
	a := aggregates.NewAgent(id, systemPrompt)
...
func Test_Agent_AddTask_With_OneTask_Should_HaveOneTask(t *testing.T) {
	// Arrange
	a := aggregates.NewAgent("test-agent", "prompt")
	task := entities.NewTask("task-1", "Test Task", "Do something")
...
func Test_Agent_AddMessage_With_OneMessage_Should_HaveOneMessage(t *testing.T) {
	// Arrange
	a := aggregates.NewAgent("test-agent", "prompt")
	msg := entities.NewMessage(immutable.RoleUser, "Hello")
...
func Test_Agent_ClearMessages_With_Messages_Should_ClearAll(t *testing.T) {
	// Arrange
	a := aggregates.NewAgent("test-agent", "prompt")
	a.AddMessage(entities.NewMessage(immutable.RoleUser, "msg1"))
	a.AddMessage(entities.NewMessage(immutable.RoleUser, "msg2"))
...
func Test_Agent_GetMessages_With_SystemPrompt_Should_IncludeSystemMessage(t *testing.T) {
	// Arrange
	a := aggregates.NewAgent("test-agent", "You are helpful")
	a.AddMessage(entities.NewMessage(immutable.RoleUser, "Hi"))
	...
	assert.That(t, "first message must be system", messages[0].Role, immutable.RoleSystem)
	assert.That(t, "second message must be user", messages[1].Role, immutable.RoleUser)
}

func Test_Agent_HasPendingTasks_With_PendingTask_Should_ReturnTrue(t *testing.T) {
	// Arrange
	a := aggregates.NewAgent("test-agent", "prompt")
	a.AddTask(entities.NewTask("task-1", "Task", "input"))
