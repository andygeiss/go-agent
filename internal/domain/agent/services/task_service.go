...
import (
	...
	"github.com/andygeiss/go-agent/internal/domain/agent/ports"
...
func NewTaskService(llmClient ports.LLMClient, executor ports.ToolExecutor, publisher event.EventPublisher) *TaskService {
	return &TaskService{
		executor:  executor,
		llmClient: llmClient,
		publisher: publisher,
...
func (s *TaskService) RunTask(ctx context.Context, agent *aggregates.Agent, task entities.Task) (entities.Result, error) {
...
func (s *TaskService) initializeTask(ctx context.Context, agent *aggregates.Agent, task entities.Task) (*entities.Task, error) {
	agent.AddTask(task)
	...
	agent.AddMessage(entities.NewMessage(immutable.RoleUser, currentTask.Input))
	return currentTask, nil
}

// executeIteration performs one iteration of the agent loop.
func (s *TaskService) executeIteration(ctx context.Context, agent *aggregates.Agent, currentTask *entities.Task) error {
...
func (s *TaskService) handleLLMError(ctx context.Context, agent *aggregates.Agent, task *entities.Task, err error) error {
	task.Fail(err.Error())
...
func (s *TaskService) executeToolCalls(ctx context.Context, agent *aggregates.Agent, task *entities.Task, toolCalls []entities.ToolCall) error {
	for _, toolCall := range toolCalls {
		tc := entities.NewToolCall(toolCall.ID, toolCall.Name, toolCall.Arguments)
		tc.Execute()
...
func (s *TaskService) finalizeTask(ctx context.Context, agent *aggregates.Agent, task *entities.Task) (entities.Result, error) {
	if !task.IsTerminal() {
		...
		return entities.NewResult(task.ID, false, "").WithError(errMsg), nil
	}
	...
	if task.IsCompleted() {
		...
		return entities.NewResult(task.ID, true, task.Output), nil
	}

	return entities.NewResult(task.ID, false, "").WithError(task.Error), nil
}

// publishTaskCompleted publishes a task completed event.
func (s *TaskService) publishTaskCompleted(ctx context.Context, agent *aggregates.Agent, task *entities.Task) error {
...
func (s *TaskService) publishTaskFailed(ctx context.Context, agent *aggregates.Agent, task *entities.Task, errMsg string) error {
...
func (s *TaskService) publishTaskStarted(ctx context.Context, agentID immutable.AgentID, task *entities.Task) error {
...
func (s *TaskService) publishToolCallExecuted(ctx context.Context, agentID immutable.AgentID, taskID immutable.TaskID, tc *entities.ToolCall) error {
